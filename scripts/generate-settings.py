...
settings_region_size = config["config"].get("settings_region_size", settings_byte_count)
if settings_region_size < settings_byte_count:
    raise ValueError(
        f"settings_region_size ({settings_region_size}) is smaller than the "
        f"generated settings map ({settings_byte_count})"
    )
...
if settings_region_size > eeprom_capacity_bytes:
    raise ValueError(
        f"settings_region_size ({settings_region_size}) exceeds EEPROM capacity "
...
erase_bus_time_seconds = settings_region_size * erase_clocks_per_byte / i2c_clock_hz
erase_typical_seconds = erase_bus_time_seconds + (
    settings_region_size * write_cycle_typical_ms / 1000
)
erase_max_seconds = erase_bus_time_seconds + (
    settings_region_size * write_cycle_max_ms / 1000
)

def write_stats_readme() -> None:
    used_percent = settings_byte_count / eeprom_capacity_bytes * 100
    reserved_percent = settings_region_size / eeprom_capacity_bytes * 100
    free_bytes = eeprom_capacity_bytes - settings_region_size
...
| Reserved settings/erase region | {settings_region_size:,} bytes ({reserved_percent:.2f}%) |
...
`settings_erase_eeprom()` performs {settings_region_size} separate byte writes. The current driver
...
- I2C bus time: `{settings_region_size} * 45 / {i2c_clock_hz:,} = {erase_bus_time_seconds * 1000:.1f} ms`
- Typical: `{erase_bus_time_seconds:.4f} s + {settings_region_size} * {write_cycle_typical_ms} ms = {erase_typical_seconds:.2f} s`
- Maximum: `{erase_bus_time_seconds:.4f} s + {settings_region_size} * {write_cycle_max_ms} ms = {erase_max_seconds:.2f} s`
...
config_c.write(f"#define EE_SIZE_SETTINGS {settings_region_size}U\n\n")
